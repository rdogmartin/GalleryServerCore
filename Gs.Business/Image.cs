		...
		public override void Inflate()
		{
			...
			if ((!this.IsNew) && (!this.IsInflated))
			{
				...
				if ((!this.IsInflated) || (this.HasChanges))
					throw new System.InvalidOperationException($"The Factory.LoadImageInstance(IGalleryObject) method should have set IsInflated=true and HasChanges=false. Instead it currently has these values: IsInflated={this.IsInflated}; HasChanges={this.HasChanges}.");
			}
		}
