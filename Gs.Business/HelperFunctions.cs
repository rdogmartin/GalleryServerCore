...
namespace GalleryServer.Business
{
    ...
    public static class HelperFunctions
    {
        ...
        public static string ValidateDirectoryName(string dirPath, string dirName, int defaultAlbumDirectoryNameLength)
        {
            ...
            {
                throw new ArgumentException($"One or both arguments contain an empty string. dirPath = \"{dirPath}\"; dirName = \"{dirName}\"");
            }
        ...
        public static string ValidateFileName(string dirPath, string fileName)
        {
            ...
            {
                throw new ArgumentException($"One or both arguments contain an empty string. dirPath = \"{dirPath}\"; fileName = \"{fileName}\"");
            }

            if (!(Path.HasExtension(fileName)))
            {
                throw new ArgumentException($"Gallery Server does not support files without an extension. (fileName = \"{fileName}\")");
            }
        ...
        public static string MapAlbumDirectoryStructureToAlternateDirectory(string albumPhysicalPath, string alternatePhysicalPath, string fullMediaObjectPath)
        {
            if (albumPhysicalPath == null)
                throw new ArgumentNullException(nameof(albumPhysicalPath));

            if (fullMediaObjectPath == null)
                throw new ArgumentNullException(nameof(fullMediaObjectPath));
        ...
        public static string CalculateFullPath(string physicalAppPath, string relativeOrFullPath)
        {
            ...
            if (String.IsNullOrEmpty(relativeOrFullPath))
                throw new ArgumentOutOfRangeException(nameof(relativeOrFullPath));

            if (String.IsNullOrEmpty(physicalAppPath))
                throw new ArgumentOutOfRangeException(nameof(physicalAppPath));
        ...
        public static void ValidatePhysicalPathExistsAndIsReadable(string fullPhysicalPath)
        {
            ...
            if (!Directory.Exists(fullPhysicalPath))
                throw new DirectoryNotFoundException($"The path {fullPhysicalPath} was not found. Check that the gallery is correctly configured and the path exists. Note: If you create a directory to resolve this error, you may have to restart the IIS application pool to get IIS to discover the directory.");
