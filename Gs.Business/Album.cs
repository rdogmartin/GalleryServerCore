    ...
    public class Album : GalleryObject, IAlbum
    {
            ...
            get
            {
                ...
                if (this.IsRootAlbum)
                {
                    if (!(String.IsNullOrEmpty(this.DirectoryName)))
                        throw new BusinessException($"DirectoryName must be empty for the root album. It was \"{this.DirectoryName}\"");
        ...
        public override void RemoveGalleryObject(IGalleryObject galleryObject)
        {
            ...
            if (!this._galleryObjects.Contains(galleryObject))
                throw new ArgumentException($"The specified IGalleryObject is not a child of this album, and thus cannot be removed. Album ID = {this.Id}; Gallery Object ID = {galleryObject.Id}; Gallery Object parent album ID = {galleryObject.Parent.Id}");
        ...
        /// <see cref="SortAscending" />, optionally persisting the changes to the database, acting recursively and - when
        ...
        public static void AssignAlbumThumbnail(IAlbum album, bool recursivelyAssignParentAlbums, bool recursivelyAssignChildrenAlbums, string userName)
        {
            ...
            if (recursivelyAssignParentAlbums)
            {
                ...
                {
                    Album.AssignAlbumThumbnail((IAlbum)albumToUpdate.Parent, recursivelyAssignParentAlbums, false, userName);
