    ...
    public static class EventController
    {
        ...
        public static IEvent RecordEvent(string msg, EventType eventType = EventType.Info, int? galleryId = null, IGallerySettingsCollection gallerySettingsCollection = null, IAppSetting appSettings = null, Dictionary<string, string> data = null)
        {
            if (galleryId == int.MinValue)
                throw new ArgumentOutOfRangeException(nameof(galleryId), string.Format("The galleryId parameter must represent an existing gallery. Instead, it was {0}", galleryId));
        ...
        internal static string GetFriendlyEnum(EventItem enumItem)
        {
            switch (enumItem)
            {
                case EventItem.EventId: return "App Event ID";
                case EventItem.EventType: return "Type";
                case EventItem.Url: return "Url";
                case EventItem.Timestamp: return "Timestamp";
                case EventItem.ExType: return "Exception Type";
                case EventItem.Message: return "Message";
                case EventItem.ExSource: return "Source";
                case EventItem.ExTargetSite: return "Target Site";
                case EventItem.ExStackTrace: return "Stack Trace";
                case EventItem.ExData: return "Data";
                case EventItem.InnerExType: return "Inner Ex Type";
                case EventItem.InnerExMessage: return "Inner Ex Message";
                case EventItem.InnerExSource: return "Inner Ex Source";
                case EventItem.InnerExTargetSite: return "Inner Ex Target Site";
                case EventItem.InnerExStackTrace: return "Inner Ex Stack Trace";
                case EventItem.InnerExData: return "Inner Ex Data";
                case EventItem.GalleryId: return "Gallery ID";
                case EventItem.HttpUserAgent: return "HTTP User Agent";
                case EventItem.FormVariables: return "Form Variables";
                case EventItem.Cookies: return "Cookies";
                case EventItem.SessionVariables: return "Session Variables";
                case EventItem.ServerVariables: return "Server Variables";
        ...
        private static void SendEmail(IEvent ev, IAppSetting appSettings, IGallerySettingsCollection gallerySettingsCollection)
        {
            ...
            if (gallerySettingsCollection == null)
                throw new ArgumentNullException(nameof(gallerySettingsCollection));
        ...
        private static bool SendMail(IEvent ev, IUserAccount user, IAppSetting appSettings, MailAddress emailSender)
        {
            ...
            try
            {
                using (var mail = new MailMessage())
                {
                    ...
                    if (string.IsNullOrEmpty(ev.ExType))
                        ...
                        mail.Subject = string.Concat("Error: ", ev.ExType);
                    ...
                    using (var smtpClient = new SmtpClient())
                    {
                        ...
                        if (!int.TryParse(appSettings.SmtpServerPort, out var smtpServerPort))
            ...
            {
                var eventMsg = string.Concat(ex2.GetType(), ": ", ex2.Message);

                if (ex2.InnerException != null)
                    eventMsg += string.Concat(" ", ex2.InnerException.GetType(), ": ", ex2.InnerException.Message);

                ev.EventData.Add(new KeyValuePair<string, string>("Cannot send e-mail", eventMsg));
            }
