...
namespace GalleryServer.Web.Controller
{
    ...
    public class AlbumTreeViewBuilder
    {
        ...
        private async Task BindAlbumToTreeview(IEnumerable<IGalleryObject> albums, TreeNode parentNode, bool expandNode)
        {
            foreach (IAlbum album in albums)
            {
                ...
                if (!String.IsNullOrEmpty(Options.NavigateUrl))
                {
                    ...
                    node.NavigateUrl = string.Concat(Options.NavigateUrl, "?aid=", album.Id.ToString(CultureInfo.InvariantCulture));
                }
        ...
        private async Task<IEnumerable<IAlbum>> GetTopAlbums()
        {
