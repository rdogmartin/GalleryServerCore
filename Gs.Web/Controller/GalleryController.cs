...
namespace GalleryServer.Web.Controller
{
    ...
    public class GalleryController
    {
        ...
        public int GetTemplateGalleryId()
        {
        ...
        public void AddGallery(Business.Gallery gallery)
        {
            if (gallery == null)
        ...
        public void DeleteGallery(Business.Gallery gallery)
        {
            if (gallery == null)
                throw new ArgumentNullException(nameof(gallery));
        ...
        public void UpdateGallery(Business.Gallery gallery)
        {
            if (gallery == null)
                throw new ArgumentNullException(nameof(gallery));
        ...
        //public async Task ProcessInstallRequest(int galleryId)
        //{
        ...
        //        await UpdateRootAlbumTitleAfterAdminCreation(galleryId);
        //        DeleteInstallFile();
        ...
        public void BeginSync(SyncOptions syncOptions)
        {
            if (syncOptions == null)
            ...
            try
            {
                album = _albumController.LoadAlbumInstance(new AlbumLoadOptions(syncOptions.AlbumIdToSynchronize) { IsWritable = true, InflateChildObjects = true });
        ...
        public SynchStatusWebEntity GetSyncStatus(string syncId, int galleryId)
        {
            ...
            try
            {
                if (!String.IsNullOrEmpty(synchStatus.SynchId) && !synchStatus.SynchId.Equals(syncId, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    synchStatusWeb.StatusForUI = "Another synchronization is in progress";
        ...
        public void AbortSync(string syncId, int galleryId)
        {
        ...
        public IUiTemplateCollection GetUiTemplates()
        {
        ...
        public async Task<GalleryData> GetGalleryDataForMediaObject(IGalleryObject mediaObject, IAlbum mediaObjectContainer, GalleryDataLoadOptions options)
        {
            ...
            //var user = await UserController.GetUserEntity(new Utils(_httpContextAccessor).UserName, mediaObject.GalleryId);
            //data.User = new User()
            //{
        ...
        public async Task<GalleryData> GetGalleryDataForAlbum(IAlbum album, GalleryDataLoadOptions options)
        {
            ...
            //// Assign user, but only grab the required fields. We do this to prevent unnecessary user data from traveling the wire.
            //var user = await UserController.GetUserEntity(new Utils(_httpContextAccessor).UserName, album.GalleryId);
            //data.User = new User()
            //{
            //    UserName = user.UserName,
            //    IsAuthenticated = user.IsAuthenticated,
        ...
        //private MediaItem GetCurrentMediaItem(GalleryData data, IGalleryObject mediaObject)
        //{
        ...
        ///// </summary>
        //private void ValidateActiveDirectoryRequirements()
        //{
        ...
        ///// </summary>
        //private void ValidateActiveDirectoryMeetsLicenseRequirements()
        //{
        ...
        ///// </summary>
        //private void ValidateActiveDirectoryRoleProviderRequirements()
        //{
        ...
        //private string CheckForDbCompactionRequest()
        //{
        //  string msg = null;
        //  bool compactDb;
        ...
        private void InsertSampleUsersAndRoles()
        {
        ...
        ///// </summary>
        //private async Task AddMembershipDataToGallerySettings()
        //{
        ...
        private string GetFriendlyStatusText(ISynchronizationStatus status)
        {
            switch (status.Status)
            {
                case SynchronizationState.AnotherSynchronizationInProgress:
                    return "Another synchronization is in progress";
                ...
                case SynchronizationState.PersistingToDataStore:
                    return $"Updating data store...{GetProgressCount(status)}";
                case SynchronizationState.SynchronizingFiles:
                    return $"Processing files...{GetProgressCount(status)}";
                case SynchronizationState.Aborted:
                    return $"canceled...{GetProgressCount(status)}";
        ...
        private int CalculatePercentComplete(ISynchronizationStatus synchStatus)
        {
        ...
        private string CalculateSyncRate(ISynchronizationStatus synchStatus)
        {
            ...
            return String.Format(CultureInfo.CurrentCulture, "{0:N1} assets/sec", (synchStatus.CurrentFileIndex / elapsedTime));
        }

        private string GetProgressCount(ISynchronizationStatus status)
        {
            var curFileIndex = (status.Status == SynchronizationState.Complete ? status.TotalFileCount : status.CurrentFileIndex);

            return $" ({curFileIndex} of {status.TotalFileCount} assets synchronized)";
        }
        ...
        /// </summary>
        /// <param name="galleryId">The ID for the gallery where the sample objects are to be created.</param>
        private void CreateSampleObjects(int galleryId)
        {
            ...
            if (sampleAlbum == null)
            {
                ...
                sampleAlbum.Caption = welcomeMsg;
            ...
            foreach (var sampleAssetFileName in Constants.SAMPLE_ASSET_FILENAMES)
            {
                ...
                if (sampleImage == null)
                {
                    ...
                    if (!File.Exists(sampleAssetFilePath))
                    {
                        ...
                        {
                            File.Move(sourceFilePath, sampleAssetFilePath);
                        }
                    }
                    else
                    {
        ...
        //private Task<IUserAccount> CreateAdministrator(int galleryId)
        //{
        //    var user = _userController.GetAdminUserFromInstallTextFile();
        ...
        //    if (_userController.MembershipGsp.GetType().ToString() == GlobalConstants.ActiveDirectoryMembershipProviderName)
        //    {
        ...
        //private async Task<IUserAccount> CreateActiveDirectoryAdministrator(User user)
        //{
        //    if (user == null)
        ...
        //    var sysAdminRole = await _roleController.ValidateSysAdminRole();
        ...
        //        if (!await _roleController.IsUserInRole(user.UserName, sysAdminRole))
        //        {
        //            await _roleController.AddUserToRole(user.UserName, sysAdminRole);
        //        }
        //    }

        //    await _roleController.CreateAuthUsersRole();
        //    if (!await _roleController.IsUserInRole(user.UserName, GlobalConstants.AuthenticatedUsersRoleName))
        //    {
        //        await _roleController.AddUserToRole(user.UserName, GlobalConstants.AuthenticatedUsersRoleName);
        //    }
        ...
        //private async Task<IUserAccount> CreateMembershipAdministrator(User user)
        //{
        //    if (user == null)
        ...
        //    var userAccount = _userController.GetUser(user.UserName, false);

        //    if (userAccount != null)
        //    {
        //        if (!_userController.MembershipGsp.ValidateUser(user.UserName, user.Password))
        //        {
        //            // Password doesn't match. Try to update.
        //            if (!_userController.EnablePasswordRetrieval)
        //            {
        ...
        //            if (!_userController.ChangePassword(user.UserName, _userController.GetPassword(user.UserName), user.Password))
        //            {
        ...
        //        _roleController.ValidateSysAdminRole();
        //        if (!await _userController.IsUserInRole(user.UserName, GlobalConstants.SysAdminRoleName))
        //        {
        //            await _roleController.AddUserToRole(user.UserName, GlobalConstants.SysAdminRoleName);
        //        }

        //        await _roleController.CreateAuthUsersRole();
        //        if (!await _userController.IsUserInRole(user.UserName, GlobalConstants.AuthenticatedUsersRoleName))
        //        {
        //            await _roleController.AddUserToRole(user.UserName, GlobalConstants.AuthenticatedUsersRoleName);
        //        }
        //    }
        ...
        //        user.Roles = new[] { await _roleController.ValidateSysAdminRole(), await _roleController.CreateAuthUsersRole() };

        //        userAccount = _userController.CreateUser(user);
        //    }
        ...
        /// </summary>
        private async Task UpdateRootAlbumTitleAfterAdminCreation(int galleryId)
        {
            ...
            updateableRootAlbum.Caption = welcomeMsg;
            await _galleryObjectController.SaveGalleryObject(updateableRootAlbum);
        }
