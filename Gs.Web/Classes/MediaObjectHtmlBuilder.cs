...
//        }
        ...
        private string GetHtmlTemplate()
        {
            ...
            if (isInQueue)
            {
                return "<p class='gsp_item_process_msg'>The item is being processed and is not yet ready for viewing.</p>";
            }
            ...
            if (isBrowserIncompatibleImage || String.IsNullOrEmpty(htmlOutput))
            {
                ...
                var url = string.Concat(GetMediaObjectUrl(), "&sa=1"); // Get URL with the "send as attachment" query string parm
                                                                       //var url = Utils.AddQueryStringParameter(GetMediaObjectUrl(), "sa=1"); // Get URL with the "send as attachment" query string parm
                var msg = $"Your browser cannot display this media asset. <a href='{url}'>Download</a>";
        ...
        private string GetPageUrl(PageId page, string format, params object[] args)
        {
            ...
            return String.Concat(HostUrl, DestinationPageUrl, queryString);
